package com.yowyob.tiibntick.core.legacydocuments.adapter.out.kernel;

import com.fasterxml.jackson.databind.JsonNode;
import com.yowyob.tiibntick.core.legacydocuments.application.port.out.IKernelLegacyDocumentGatewayPort;
import com.yowyob.tiibntick.core.legacydocuments.domain.model.KernelApiEnvelope;
...
/**
 * Calls the Kernel's {@code billing-legacy-documents-controller} HTTP surface via the
 * shared {@code kernelWebClient} bean (defined once in {@code tnt-bootstrap}'s
 ...
 */
@Component
public class KernelLegacyDocumentGatewayAdapter implements IKernelLegacyDocumentGatewayPort {

    private final WebClient kernelWebClient;

    public KernelLegacyDocumentGatewayAdapter(@Qualifier("kernelWebClient") WebClient kernelWebClient) {
        this.kernelWebClient = kernelWebClient;
    }

    @Override
    public Mono<KernelDocumentResult> invoke(HttpMethod method, String kernelPath, MultiValueMap<String, String> queryParams,
        ...
        return withBody.exchangeToMono(response -> response.bodyToMono(KernelApiEnvelope.class)
                ...
                .map(envelope -> new KernelDocumentResult(response.statusCode().value(), envelope))
                .onErrorResume(ex -> Mono.just(new KernelDocumentResult(
