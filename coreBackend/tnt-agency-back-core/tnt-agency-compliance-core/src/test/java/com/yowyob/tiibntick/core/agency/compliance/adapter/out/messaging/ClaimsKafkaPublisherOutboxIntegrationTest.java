 ...
 * {@link ClaimsKafkaPublisher} now enqueues claim submissions into the {@code yow-event-kernel}
 * transactional outbox instead of sending to Kafka directly, and {@code OutboxPollerService}
 * relays them asynchronously — on the exact same topic and flat wire payload format
 * ({@code claimRef, tenantId, agencyId, missionId, claimType, description, contactEmail}) that
 * existing consumers already expect.
 *
 * <p>Uses a real PostgreSQL Testcontainer (event_bus schema migrated via
 * {@code yow-event-kernel}'s own Liquibase changelog) and an embedded Kafka broker — same
 * pattern as {@code KafkaDeliveryEventPublisherOutboxIntegrationTest} in tnt-delivery-core, the
 * pilot migration this one replicates.
 */
@SpringBootTest(classes = ClaimsKafkaPublisherOutboxIntegrationTest.TestConfig.class)
@Testcontainers
@EmbeddedKafka(partitions = 1, topics = {ClaimsKafkaPublisherOutboxIntegrationTest.CLAIMS_TOPIC})
@Tag("integration")
class ClaimsKafkaPublisherOutboxIntegrationTest {

    static final String CLAIMS_TOPIC = "tnt.agency.claims.submitted";

    @Container
    @SuppressWarnings("resource")
    static final PostgreSQLContainer<?> POSTGRES = new PostgreSQLContainer<>("postgres:16-alpine")
            .withDatabaseName("tnt_compliance_outbox_test")
    ...
    @Autowired
    private ClaimsKafkaPublisher publisher;
    ...
    @Test
    @DisplayName("submitClaim() enqueues to the outbox (PENDING, not yet on Kafka); the poller then "
            + "delivers it to the exact topic/payload format consumers expect")
    void submitClaimEnqueuesToOutboxThenPollerDeliversToKafka() throws Exception {
        ObjectMapper objectMapper = new ObjectMapper().registerModule(new JavaTimeModule());

        UUID tenantId = UUID.randomUUID();
        UUID agencyId = UUID.randomUUID();
        UUID missionId = UUID.randomUUID();
        ClaimEventPublisherPort.ClaimSubmission submission = new ClaimEventPublisherPort.ClaimSubmission(
                tenantId, agencyId, missionId, "DAMAGE", "Parcel arrived damaged", "client@example.com");

        String claimRef = publisher.submitClaim(submission).block(Duration.ofSeconds(10));
        assertThat(claimRef).startsWith("CLM-");

        // 1. The envelope is durably persisted PENDING — nothing sent to Kafka yet.
        StepVerifier.create(envelopeRepository.findByAggregateId(claimRef, "Claim", tenantId.toString()))
                .assertNext(envelope -> {
                    assertThat(envelope.getKafkaTopic()).isEqualTo(CLAIMS_TOPIC);
        ...
        Map<String, Object> consumerProps = new HashMap<>(
                KafkaTestUtils.consumerProps(embeddedKafka, "compliance-outbox-it", true));
        consumerProps.put(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG, StringDeserializer.class);
        consumerProps.put(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG, StringDeserializer.class);
        consumerProps.put(ConsumerConfig.AUTO_OFFSET_RESET_CONFIG, "earliest");

        try (Consumer<String, String> rawConsumer = new KafkaConsumer<>(consumerProps)) {
            embeddedKafka.consumeFromAnEmbeddedTopic(rawConsumer, CLAIMS_TOPIC);
            ...
            ConsumerRecord<String, String> received =
                    getSingleRecord(rawConsumer, CLAIMS_TOPIC, Duration.ofSeconds(15));

            assertThat(received.topic()).isEqualTo(CLAIMS_TOPIC);
            assertThat(received.key()).isEqualTo(tenantId.toString());

            JsonNode wire = objectMapper.readTree(received.value());
            assertThat(wire.get("claimRef").asText()).isEqualTo(claimRef);
            assertThat(wire.get("tenantId").asText()).isEqualTo(tenantId.toString());
            assertThat(wire.get("agencyId").asText()).isEqualTo(agencyId.toString());
            assertThat(wire.get("missionId").asText()).isEqualTo(missionId.toString());
            assertThat(wire.get("claimType").asText()).isEqualTo("DAMAGE");
            assertThat(wire.get("description").asText()).isEqualTo("Parcel arrived damaged");
            assertThat(wire.get("contactEmail").asText()).isEqualTo("client@example.com");
        }
    ...
    static class TestConfig {
        ...
        @Bean
        ClaimsKafkaPublisher claimsKafkaPublisher(PublishEventUseCase publishEventUseCase) {
            return new ClaimsKafkaPublisher(publishEventUseCase, new ObjectMapper(), CLAIMS_TOPIC);
        }
    }
}
