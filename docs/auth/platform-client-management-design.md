...
**Post-launch incident, 2026-07-09 → 2026-07-25:** `PlatformClientEntity`/`ApiKeyEntity`/`ClientPermissionEntity`/`ApiKeyRotationHistoryEntity`/`ClientAuditLogEntity` all assign their `@Id` client-side (`UUID.randomUUID()`) before `save()`. Spring Data R2DBC's default `isNew()` check treated that as "already exists" and issued a no-op `UPDATE` instead of an `INSERT` on every single create call — `POST /api/v1/admin/platform-clients` returned `201 Created` with a fully-populated body while persisting nothing, for the entire 2.5 weeks between this module's launch and the fix. Fixed by implementing `Persistable<String>` on the affected entities (commit `efdecebb`). See `docs/architecture/decisions.md` ADR-020 and `docs/knowledge/known-issues.md` #18 for the full write-up, and `docs/auth/platform-client-onboarding-guide.md` §10 for current production status (5 platform clients live as of 2026-07-25).
