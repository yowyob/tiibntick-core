...
# Summary
**As of 2026-07-31**: the full reactor builds and boots cleanly, Kernel is HTTP-only end-to-end (zero `yowyob.comops.api:RT-comops-*` Maven dependencies anywhere in the repo), Phase 0 of the audit remediation plan is substantially closed (see `docs/audits/remediation/phase-0-critical.md` for the authoritative, checkbox-tracked status — Chantiers A/B/C/G done, D/E/F have real, documented residual gaps), a DB-backed platform Client-ID/API-Key system is live in production with 5 platform identities issued, and `tnt-incident-core`/`tnt-dispute-core` now have real RBAC + actor-ownership scoping (previously incident-core had none at all, dispute-core only `isAuthenticated()` — see `knowledge/known-issues.md` #26). No known build-blocking issues remain.
...
| `tnt-incident-core`/`tnt-dispute-core` RBAC | ✅ as of 2026-07-31 — both now `@RequirePermission`-gated (`incident:*`/`dispute:*`) with actor-ownership scoping; previously incident-core had zero access control, dispute-core only `isAuthenticated()` — see `knowledge/known-issues.md` #26, `security/roles.md` |
...
1. **2026-07-31**: Go-Freelancer/TiiBnTick Co-Freelancer frontend integration review found `tnt-incident-core` had zero access control and `tnt-dispute-core` only `isAuthenticated()` (any freelancer JWT could reach mediator/manager-tier actions on any tenant's data). Fixed: both modules now depend on `tnt-roles-core`; new `incident:create/read/manage` permissions; every application-service method `@RequirePermission`-gated; non-privileged callers scoped to records they reported/are a party to (`IncidentRequesterContext`, `GetDisputeQuery`/`ListDisputesQuery` `requesterId`/`privileged`); forgeable `X-Actor-ID` headers replaced with `@CurrentUser`; response DTOs gained FreelancerOrg-context fields; 3 previously-untyped incident endpoints and 3 dispute freelancer-org endpoints now return typed DTOs. See `knowledge/known-issues.md` #26.
2. **2026-07-25**: tracking codes wired end-to-end; GPS-to-Kalman ETA pipeline made real (previously a dead stub); R2DBC `Persistable` bug fixed (had silently blocked all platform-client creation since 2026-07-09); 5 platform-client identities issued in prod.
