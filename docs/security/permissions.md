...
Public `String` constants in `TntPermission.java`, format `resource:action`. Categories: `MISSION_*`, `DELIVERY_*`, `BILLING_*`, `INVOICE_*`, `WALLET_*`, `AGENCY_*`, `BRANCH_*`, `ACTOR_*`, `TRUST_*`, `RELAY_*`, `ANNOUNCEMENT_*`, `DISPUTE_*`, `INCIDENT_*`, `REPORT_*`, `RESOURCE_*`, `PRODUCT_*`, `INVENTORY_*`, `SETTINGS_*`, `ROUTE_*`, `GEO_*`, `MEDIA_*`, `PAYMENT_*`, `ADMIN_*`, `PLATFORM_CLIENTS_MANAGE` (`platform:clients`), plus `ALL` (`"*"`, TNT_ADMIN only). Always import the constant — never hardcode a permission string inline. See `api/security.md` for endpoint-level usage.

`INCIDENT_CREATE`/`INCIDENT_READ`/`INCIDENT_MANAGE` (`incident:create`/`incident:read`/`incident:manage`) were added 2026-07-31 as part of Go-Freelancer integration hardening — `tnt-incident-core` previously shipped with **zero** `@RequirePermission`/`@PreAuthorize` on any endpoint (any authenticated JWT of any role could triage/escalate/resolve/close any tenant's incidents). See `security/roles.md`'s "Incident/dispute permissions" section for the per-role grant and the ownership-scoping model layered on top of the plain permission gate.

`PLATFORM_CLIENTS_MANAGE` is deliberately not added to any `TntRole`'s `defaultPermissions` — only `TNT_ADMIN`'s `ALL` (`*`) wildcard satisfies it, making the `tnt-platform-gateway-core` admin API exclusive to the system administrator by construction (see `docs/auth/platform-client-management-design.md` §4).

## Shared matching engine (2026-07-09)
`TntPermissionEvaluator.matches()`'s base exact/`resource:*`/`*` matching now delegates to `com.yowyob.tiibntick.common.security.PermissionMatcher` (`tnt-common-core`) — a small, dependency-free utility. `tnt-platform-gateway-core`'s platform-client M2M scope model (a *different* permission axis — which platform backend may call what, not which user may do what) uses the same `PermissionMatcher` for its own `resource:action` scope strings, so both RBAC and platform scopes share one definition of what a wildcard means. `TntPermissionEvaluator` still owns the `#SCOPE`-suffix (`#AGENCY:<id>`/`#SYSTEM`/`#TENANT`) semantics on top — that layer is user-RBAC-specific and does not apply to platform scopes.
