 ...
 * (see {@code CORE_KERNEL_GATEWAY_SPEC.md} §6) — only the base URL changes.
 *
 * <p>Registered as public (no TiiBnTick JWT required) in {@code TntSecurityConfig} —
 * these ARE the bootstrap endpoints platforms call before they hold one. The caller's
 * own {@code Authorization} header (if any — relevant for {@code mfa/disable},
 * {@code users/{userId}/reset-password}) is forwarded to the Kernel unchanged; the
 * Kernel remains the sole authority on whether it's valid/sufficient.
 *
