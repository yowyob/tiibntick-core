 ...
 * against the Kernel's {@code auth-oidc-controller} (see
 * {@code CORE_KERNEL_GATEWAY_SPEC.md} §7). Unlike {@link ProxyKernelAuthUseCase}, this is
 * real orchestration logic (interpreting the userinfo payload, building the token-exchange
 * grant), not a raw proxy — so the contracts are typed rather than opaque JSON.
 *
...
public interface ProxyKernelSsoUseCase {
     ...
     * app's SSO redirect URL (see {@code tnt.auth.platform-gateway.sso-app-redirect-urls}).
     */
