 ...
 * field-name variants when walking {@code contexts[].organizations[]}. Adjust
 * {@link #findMatchingContext} if the live field names differ.
 *
...
public class KernelSsoGatewayService implements ProxyKernelSsoUseCase {
    ...
    public Mono<SsoLaunchResponse> launch(SsoLaunchRequest request) {
        ...
        if (redirectBase == null) {
            return Mono.error(TntPlatformGatewayException.ssoAppNotConfigured(request.app()));
        }
    ...
    private Mono<ResolveSsoContextResponse> parseUserInfo(KernelRawResponse response, String kernelOrganizationId) {
        if (!isSuccess(response)) {
            return Mono.error(TntPlatformGatewayException.ssoContextNotFound(kernelOrganizationId));
        }
        return findMatchingContext(response, kernelOrganizationId)
                .map(Mono::just)
                .orElseGet(() -> Mono.error(TntPlatformGatewayException.ssoContextNotFound(kernelOrganizationId)));
    }
    ...
    private Mono<SsoTokenExchangeResponse> parseTokenResponse(KernelRawResponse response) {
        if (!isSuccess(response)) {
            return Mono.error(TntPlatformGatewayException.ssoTokenExchangeFailed("Kernel returned HTTP " + response.status()));
        }
        try {
            JsonNode root = objectMapper.readTree(response.body());
            String token = firstNonBlank(textOrNull(root, "access_token"), textOrNull(root, "accessToken"));
            if (token == null) {
                return Mono.error(TntPlatformGatewayException.ssoTokenExchangeFailed("no access_token in Kernel response"));
            }
            return Mono.just(new SsoTokenExchangeResponse(token));
        } catch (Exception e) {
            return Mono.error(TntPlatformGatewayException.ssoTokenExchangeFailed(e.getMessage()));
        }
    }
