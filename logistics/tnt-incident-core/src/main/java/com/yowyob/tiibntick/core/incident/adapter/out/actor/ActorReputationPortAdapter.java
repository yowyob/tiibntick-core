package com.yowyob.tiibntick.core.incident.adapter.out.actor;
...
/**
 * tnt-incident-core implementation of its own {@link IActorReputationPort} outbound port.
 *
 * <p>{@code tnt-incident-core} (L3) depends down into {@code tnt-actor-core} (L2) purely to
 * implement this adapter — the same one-directional pattern used by {@code tnt-trust-core}
 * for {@code IBlockchainAuditPort}: the calling module ({@code tnt-incident-core}) owns the
 * outbound port interface, and this adapter, which needs a capability from a lower layer,
 * lives in the calling module itself and reaches down into it. {@code tnt-actor-core} never
 * depends back on {@code tnt-incident-core}.
 *
 * <p>This adapter is used internally by {@code tnt-incident-core}'s own services to apply
 * reputation consequences when incidents are closed or escalated to fraud disputes. It
 * translates incident-domain operations into actor-domain mutations:
 * <ul>
